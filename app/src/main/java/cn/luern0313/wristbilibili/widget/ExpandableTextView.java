...
public class ExpandableTextView extends TextView
{
    ...
    public void setOrigText(CharSequence text)
    {
        this.mOrigText = text;
    }
    ...
    private CharSequence getNewTextByConfig()
    {
        ...
        switch (mCurrState)
        {
            case STATE_SHRINK:
            {
                ...
                int indexEndTrimmed = indexEnd - getLengthOfString(
                        mEllipsisHint) - (mShowToExpandHint ? getLengthOfString(
                        mToExpandHint) + getLengthOfString(mGapToExpandHint) : 0);
                ...
                float widthTailReplaced = mTextPaint.measureText(getContentOfString(
                        mEllipsisHint) + (mShowToExpandHint ? (getContentOfString(
                        mToExpandHint) + getContentOfString(mGapToExpandHint)) : ""));
                ...
                if(remainWidth > widthTailReplaced)
                {
                    ...
                    while (remainWidth > widthTailReplaced + extraWidth)
                    {
                        ...
                        {
                            extraWidth = (int) (mTextPaint.measureText(mOrigText.subSequence(
                                    indexEndTrimmed, indexEndTrimmed + extraOffset)
                                                                               .toString()) + 0.5);
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    while (remainWidth + extraWidth < widthTailReplaced)
                    {
                        ...
                        {
                            extraWidth = (int) (mTextPaint.measureText(mOrigText.subSequence(
                                    indexEndTrimmed + extraOffset, indexEndTrimmed)
                                                                               .toString()) + 0.5);
                        }
                        else
                        {
                ...
                //SpannableStringBuilder ssbShrink = new SpannableStringBuilder(
                        //mOrigText.subSequence(0, indexEndTrimmedRevised)).append(mEllipsisHint);
            ...
            case STATE_EXPAND:
            {
                ...
                SpannableStringBuilder ssbExpand = new SpannableStringBuilder(mOrigText).append(
                        mGapToShrinkHint).append(mToShrinkHint);
    ...
    private View.OnClickListener getOnClickListenerV14(View view)
    {
        ...
        try
        {
            ...
            Field clickListenerField = Class.forName(CLASS_NAME_LISTENER_INFO).getDeclaredField(
                    "mOnClickListener");
    ...
    private class TouchableSpan extends ClickableSpan
    {
        ...
        {
            if(hasOnClickListeners() && (getOnClickListener(
                    ExpandableTextView.this) instanceof ExpandableClickListener))
            {
            }
            else
            {
        ...
        public boolean onTouchEvent(TextView textView, Spannable spannable, MotionEvent event)
        {
            if(event.getAction() == MotionEvent.ACTION_DOWN)
            {
                ...
                if(clickableSpan != null)
                {
                    ...
                    {
                        mPressedSpan = (TouchableSpan) clickableSpan;
            ...
            else if(event.getAction() == MotionEvent.ACTION_MOVE)
            {
                ...
                if(clickableSpan != null)
                {
                    if(clickableSpan instanceof TouchableSpan)
                    {
                        TouchableSpan touchedSpan = (TouchableSpan) clickableSpan;
            ...
            else if(event.getAction() == MotionEvent.ACTION_UP)
            {
                ...
                else if(clickableSpan instanceof LinkSpan)
                {
                    Intent intent = new Intent(MyApplication.getContext(), UnsupportedLinkActivity.class);
                    intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
                    intent.putExtra("url", ((LinkSpan) clickableSpan).getUrl());
